...
const normalMarkers = await page.locator('svg [data-wmo][data-lon]').count();
...
for (const w of testedWmos) {
  const el = page.locator(`svg [data-wmo="${w}"][data-lon]`).first();
...
async function markerXY(w) {
  const el = page.locator(`svg [data-wmo="${w}"][data-lon]`).first();
  const halo = el.locator('circle').nth(1); // halo circle carries cx,cy
  return { x: Number(await halo.getAttribute('cx')), y: Number(await halo.getAttribute('cy')) };
}
...
await sleep(300);
const postWheel = await markerXY(wmoA);
...
await page.getByTestId('map-zoom-in').click();
await sleep(300);
const preDrag = await markerXY(wmoA);
...
await page.evaluate((w) => {
  const g = document.querySelector(`svg [data-wmo="${w}"][data-lon]`);
