// Probe 5: stamp DOM identity across toggles to detect subtree remounts.
...
await page.evaluate(() => {
  window.__svg = document.querySelector('svg[role="img"]');
  window.__marker = document.querySelector('svg[role="img"] g[data-wmo]');
  window.__slot = document.querySelector('svg[role="img"]').parentElement;
});
async function ident(tag) {
  console.log(tag, await page.evaluate(() => ({
    svgSame: document.querySelector('svg[role="img"]') === window.__svg,
    markerSame: document.querySelector('svg[role="img"] g[data-wmo]') === window.__marker,
    slotSame: document.querySelector('svg[role="img"]').parentElement === window.__slot,
  })));
}
await toggle.click(); // OFF
await sleep(1200);
await ident('afterOFF ');
