"""Float Status: latest published profile recency from Ifremer/Argo GDAC.
...
STATE_VERSION = 2
PARSER_VERSION = 4
PROFILE_RECENCY_VERSION = 2
#: Documented fallback only. Every float's expected interval is measured from
#: its own published profile history (service/profile_cycle.py); 10 days is
#: used when that history is insufficient, never as an assumption.
PROFILE_INTERVAL_DAYS = profile_cycle.DEFAULT_INTERVAL_DAYS
COMMUNICATION_FIELDS = ("JULD_LAST_MESSAGE", "JULD_TRANSMISSION_END")
#: Bounded ring of one-per-cycle verified fixes retained per float (NOT a
#: history database: same cache file, bounded width, no separate store).
#:
#: Size matters scientifically, not cosmetically. The documented Stage-0 prior
#: needs neighbouring transitions from OTHER floats (2 deg / +-1 month / >= 5
#: neighbours / 2-day delivery lag). At 8 retained cycles the fleet pool held
#: only 200 transitions and EVERY float was denied the prior -> rung 4 fell
#: through to trajectory_extrapolation. Rebuilding the same cached fields from
#: the same Ifremer files with the full available history (longest monitored
#: float: 361 one-per-cycle fixes) gives a pool of 5,294 transitions and lets
#: the documented ladder select history_prior where its conditions are met.
#: No rule, weight, radius or rung definition changes — only how much of the
#: float's own already-downloaded trajectory is kept in the cache. 400 covers
#: every monitored float's complete usable history with a fixed bound.
PREDICTION_FIX_HISTORY = 400
...
def profile_data_status(days_since: float | None, interval_days: float | None = None) -> str:
...
def parse_rtraj(raw: bytes, wmo: int, now: datetime | None = None) -> dict[str, Any]:
    ...
    as last transmission. Missing message metadata is nonfatal and never
    determines profile data status.
    """
    now = now or _utcnow()
    try:
    ...
    try:
        ...
        for i in range(min(juld.size, lat.size, lon.size)):
            ...
            fixes.append(fix)
            if out["last_fix"] is None or value > out["last_fix"]["juld"]:
                out["last_fix"] = fix
        # Bounded, current-state feature set for the Next Profile Location
        # prediction: one fix per cycle (earliest of that cycle), newest last.
        # This is a fixed-width ring of recent observations, not a store of
        # historical profile data.
        by_cycle: dict[int, dict[str, Any]] = {}
        for fix in sorted(fixes, key=lambda f: f["juld"]):
            key = fix["cycle"] if fix["cycle"] is not None else -1
            if key not in by_cycle:
                by_cycle[key] = fix
        ordered = [by_cycle[k] for k in sorted(by_cycle)]
        out["recent_fixes"] = [
            {
                "juld": f["juld"],
                "juld_iso": f["juld_iso"],
            ...
            for f in ordered[-PREDICTION_FIX_HISTORY:]
        ]
        out["cycle_intervals"] = profile_cycle.summarize_intervals(
            [f["juld"] for f in out["recent_fixes"]]
        )
        # Parking depth and cycle-phase durations are prediction features only:
        # they never enter any profile-recency formula.
        park: list[float] = []
        if "REPRESENTATIVE_PARK_PRESSURE" in ds.variables:
            for value in _farr(ds.variables["REPRESENTATIVE_PARK_PRESSURE"]):
                if np.isfinite(value) and 0 < value < 6000:
                    park.append(float(value))
        out["park_pressure_dbar"] = float(np.median(park)) if park else None
        out["park_pressure_samples"] = len(park)
        out["phase_hours"] = _phase_durations(ds)
        return out
    finally:
        ds.close()


#: (start field, end field, label) pairs read from Rtraj cycle-phase times.
_PHASE_PAIRS = (
    ("JULD_DESCENT_START", "JULD_DESCENT_END", "descent_h"),
    ("JULD_DESCENT_END", "JULD_PARK_END", "park_h"),
    ("JULD_DESCENT_END", "JULD_ASCENT_START", "park_h"),
    ("JULD_ASCENT_START", "JULD_ASCENT_END", "ascent_h"),
)


def _phase_durations(ds: netCDF4.Dataset) -> dict[str, float]:
    """Median cycle-phase durations (hours) from whichever phase times exist.

    Best-effort and prediction-only: a missing phase variable simply yields no
    entry. Values are bounded to a plausible 0-720 h window.
    """
    out: dict[str, list[float]] = {}
    for start_name, end_name, label in _PHASE_PAIRS:
        if start_name not in ds.variables or end_name not in ds.variables:
            continue
        try:
            starts, ends = _julds(ds, start_name), _julds(ds, end_name)
        except FloatSyncError:
            continue
        values = []
        for a, b in zip(starts, ends):
            if not (np.isfinite(a) and np.isfinite(b)):
                continue
            hours = (float(b) - float(a)) * 24.0
            if 0 < hours <= 720:
                values.append(hours)
        if values:
...
def parse_profile(
    ...
    try:
        ...
        if "JULD" in ds.variables and ds.variables["JULD"].dimensions != ("N_PROF",):
        ...
        # Observed cycle spacing over the same published profile dates that
        # drive monitoring. Retained as aggregate statistics only (count and
        # median/min/max) — no per-profile history is stored locally.
        out["juld_intervals"] = profile_cycle.summarize_intervals([v for _, v in valid_j])
...
class FleetSyncRegistry:
    ...
    def source_info(self) -> dict[str, Any]:
        return {
            ...
            # Fallback constant only: each float's expected interval is measured
            # from its own published profile dates (then trajectory cycles).
            "expected_profile_interval_days": PROFILE_INTERVAL_DAYS,
            "interval_policy": (
                "Expected next profile = last profile JULD + observed cycle interval "
                "(median spacing of valid profile JULDs; trajectory cycle spacing if the "
                "profile history is too short; 10-day nominal cycle only as fallback)"
            ),
            "approximation_note": (
            ...
            "profile_policy": (
                ...
                "inventory gaps = absent file cycles in 1..Prof# (not Approx. Profiles Missed)"
            ),
                ...
                self._sync.update(
                    {
                        ...
                        if failed == 0
                            ...
                            f"{failed} float(s) had source refresh failures "
                            "(kept last valid affected data)"
                        ),
                        "counts": {
                            "total": len(wmos),
                            "updated": updated,
    ...
    def _refresh_float(self, ftp: ftplib.FTP, wmo: int, members: set[str]) -> bool:
        ...
        try:
            ...
            reusable = (
                parser_current
                and aggregate
                and (old or {}).get("profile_recency_version") == PROFILE_RECENCY_VERSION
                and (old or {}).get("profile_aggregate_fp") == fp
                and aggregate.get("file") == profile_name
        ...
        try:
            ...
            if not (
                parser_current
                and (old or {}).get("profiles") is not None
                and (old or {}).get("profiles_fp") == fp
            ):
    ...
    # -- prediction input advisories --------------------------------------
    @staticmethod
    def _step_advisory(features: Any) -> str | None:
        """Report when the extrapolation step is not cycle-scale (reporting only).

        ``trajectory_extrapolation`` and the blend both apply the float's last
        observed hop as one cycle. Some floats publish several cycles within a
        day (telemetry bursts / numbering jumps), so their last hop can be a
        fraction of a cycle while the positions jump wildly. The method, rung,
        radius and stratum are returned unchanged — this only tells the operator
        that the input step may not represent a full cycle.
        """
        if features is None:
            return None
        last = features.last_transition
        interval = getattr(features, "interval_days", None)
        if last is None or not interval or interval <= 0:
            return None
        ratio = float(last.days) / float(interval)
        if 0.5 <= ratio <= 2.0:
            return None
        return (
            f"most recent observed hop is {last.days:.2f} d = {ratio:.2f}x the observed "
            f"cycle interval ({interval:.2f} d); the step used by this method may not "
            "represent one full cycle"
        )

    # -- prediction diagnostics -------------------------------------------
    @staticmethod
    def _prediction_diagnostics(floats: list[dict[str, Any]]) -> dict[str, Any]:
        """Distribution of the served Stage-0 predictions (reporting only)."""
        ...
        for row in floats:
            ...
            rung = prediction.get("fallback_rung")
            if rung is not None:
                rungs[str(rung)] = rungs.get(str(rung), 0) + 1
            status = str(prediction.get("status") or "unknown")
            statuses[status] = statuses.get(status, 0) + 1
            count = prediction.get("trajectory_transitions")
        ...
        return {
            ...
            "rungs": rungs,
            "statuses": statuses,
            "persistence_only": methods.get("persistence", 0),
            "history_prior": methods.get("history_prior", 0),
            "trajectory_extrapolation": methods.get("trajectory_extrapolation", 0),
            "regional_prior": methods.get("regional_prior", 0),
            "unavailable": sum(1 for r in floats if not (r.get("prediction") or {}).get("available")),
            "insufficient_validation": statuses.get("insufficient_validation", 0),
            "insufficient_data": statuses.get("insufficient_data", 0),
            "trajectory_history_available": history_available,
            ...
            "note": (
                "distribution of the served predictions; persistence_only counts floats whose "
                "documented prior was unavailable, not a rule change"
            ),
        }
    ...
    def payload(self) -> dict[str, Any]:
        ...
        # Next-profile-location predictions are computed from the same cached
        # upstream data (no extra downloads, no browser-side requests) and are
        # attached per float. A failure here must never break the status rows.
        try:
            from prediction import predict_fleet as _predict_fleet
            from prediction.calibrate import load_calibration as _load_calibration
            from prediction.features import build_fleet_features as _build_features
            from prediction.forward_test import get_log as _forward_log
            from prediction.predict import calibration_summary as _calibration_summary

            calibration = _load_calibration()
            features = _build_features(rows, meta or {}, now)
            features_meta = features
            predictions = _predict_fleet(features, calibration=calibration, now=now)
            # Forward test: record what was predicted and score predictions whose
            # next verified fix has since arrived. Audit only; never fatal.
            forward_test = _forward_log().observe(features, predictions, now=now)
            prediction_meta = {
            ...
            predictions = {}
            features_meta = {}
            prediction_meta = {"error": f"{type(exc).__name__}: {exc}"}
        floats = [
            self._serve_row(
                w,
                rows.get(str(w)),
        ...
        prediction_summary = {
            "available": sum(1 for f in floats if (f.get("prediction") or {}).get("available")),
            "unavailable": sum(1 for f in floats if not (f.get("prediction") or {}).get("available")),
            "methods": {},
            "calibration": prediction_meta.get("calibration"),
            "forward_test": prediction_meta.get("forward_test"),
            "error": prediction_meta.get("error"),
            "currents": (floats[0].get("prediction") or {}).get("currents") if floats else None,
            "label": "Experimental prediction — not a communication signal",
        }
        for row in floats:
            method = (row.get("prediction") or {}).get("method")
            if method:
                prediction_summary["methods"][method] = (
                    prediction_summary["methods"].get(method, 0) + 1
                )
            # A float's own forward-test record, drawn from the same audit file:
            # how many of ITS previous predictions were later checked, and how
            # they scored. Absent when nothing has been scored for it yet.
            per_float = (prediction_meta.get("forward_test") or {}).get("floats") or {}
            entry = per_float.get(str(row["wmo"]))
            if isinstance(entry, dict) and row.get("prediction") is not None:
                row["prediction"]["forward_test"] = entry
            # Advisory about the *input* step behind an available prediction.
            advisory = self._step_advisory((features_meta or {}).get(row["wmo"]))
            if advisory and (row.get("prediction") or {}).get("available"):
                row["prediction"]["issues"] = list(row["prediction"].get("issues") or []) + [advisory]
        # Fleet-wide diagnostics for the Stage-0 audit: method and fallback-rung
        # distribution, how many floats are stuck on persistence or refused, and
        # how much trajectory history each float actually contributes. Pure
        # reporting over the rows above — no rule, weight or radius is derived
        # here.
        prediction_summary["diagnostics"] = self._prediction_diagnostics(floats)
        summary = {
            "total": len(floats),
            ...
            "approx_profiles_missed_total": (
                ...
                if any(f["approx_profiles_missed"] is not None for f in floats)
                else None
            ),
    ...
    def _serve_row(
        ...
        out: dict[str, Any] = {
            ...
            "profile_history_lagging_latest_file": None,
            "profile_checked_at": None,
            "days_since_last_profile": None,
            "expected_next_profile_iso": None,
            "expected_interval_days": None,
            "expected_interval_source": None,
            "expected_interval_samples": None,
            "interval_note": None,
        ...
        if row.get("in_incois_dac") is False:
            out["profile_date_error"] = (
                row.get("reason") or "No profile history in Ifremer dac/incois"
            )
        ...
        out["position_error"] = (
            ...
            else parsed.get("position_error") or aggregate.get("position_error")
        )
        ...
        # Monitoring authority is strictly <WMO>_prof.nc:JULD. Do not use
        ...
        source_valid = (
            not old_parser
            and aggregate.get("file") == f"{wmo}_prof.nc"
            and aggregate.get("platform_number") == str(wmo)
        )
        value = _valid_juld(aggregate.get("juld"), now) if source_valid else None
        # Observed cycle interval: profile-date spacing first (the monitoring
        # source), then trajectory cycle spacing, then the documented 10-day
        # fallback. Never assumed from the nominal Argo cycle.
        interval_days, interval_source, interval_samples = profile_cycle.resolve_interval(
            aggregate.get("juld_intervals") if source_valid else None,
            rtraj.get("cycle_intervals"),
        )
        out.update(
            expected_interval_days=interval_days,
            expected_interval_source=interval_source,
            expected_interval_samples=interval_samples,
            interval_note=profile_cycle.interval_note(interval_days, interval_source, interval_samples),
        )
        if value is not None:
            latest_date = juld_to_datetime(value)
            days = (now - latest_date).total_seconds() / 86400.0
            out.update(
                last_profile_iso=latest_date.isoformat(),
                last_profile_juld=value,
                ...
                expected_next_profile_iso=(
                    latest_date + timedelta(days=interval_days)
                ).isoformat(),
                days_since_last_profile=days,
                approx_profiles_missed=profile_cycle.approx_profiles_missed(days, interval_days),
                data_status=profile_data_status(days, interval_days),
            )
            # Provenance notice only: the explicitly configured aggregate
            # remains the sole authority for all monitoring calculations.
            individual_date = _valid_juld(parsed.get("juld"), now)
            if individual_date is not None:
                out["profile_history_lagging_latest_file"] = (
                    juld_to_datetime(individual_date) > latest_date
                )
        else:
            out["profile_date_error"] = row.get("profile_sync_error") or (
                f"No valid profile JULD in {wmo}_prof.nc"
                if source_valid
                else f"No verified {wmo}_prof.nc history available; no alternate timestamp is used"
            )
        out["prediction"] = prediction
        return out
