...
The 32-record tests read the repository's existing, unmodified cache; they do
...
def test_all_32_existing_float_cache_records(wmo):
    ...
    if (
        r.get("in_incois_dac") is False
        or (r.get("rtraj") or {}).get("last_tx_field") not in fs.COMMUNICATION_FIELDS
    ):
        assert got["status"] == "NO DATA"
        for key in ("last_tx_iso", "expected_next_iso", "days_since_last_tx"):
            assert got[key] is None
        assert got["communication_error"]
    else:
        tx = datetime.fromisoformat(got["last_tx_iso"])
        assert datetime.fromisoformat(got["expected_next_iso"]) - tx == timedelta(days=10)
        days = (AS_OF - tx).total_seconds() / 86400
        assert got["days_since_last_tx"] == days
        want = "ACTIVE" if days <= 10 else "OVERDUE" if days < 60 else "NO COMMUNICATION 60+ DAYS"
...
def test_actual_cache_total_is_available_cycles_not_sum_of_maxima():
    ...
    assert sum(r["status"] == "NO DATA" for r in rows) == 4
...
def test_nonfinite_and_negative_elapsed_days_are_not_active(value):
    assert fs.comm_status(value) == "NO DATA"
...
def test_status_boundaries_and_elapsed_days_do_not_round_early(days, want):
    ...
    assert got["status"] == want
    assert got["days_since_last_tx"] == pytest.approx(days, abs=1e-11)


@pytest.mark.parametrize("iso", [None, "bad", "2026-09-01T00:00:00", "2026-09-21T00:00:01+00:00"])
def test_bad_naive_and_future_cache_timestamps_are_unavailable(iso):
    got = fs.FleetSyncRegistry._serve_row(
        2909999, _row({"last_tx_field": "JULD_LAST_MESSAGE", "last_tx_iso": iso}), {}, {}, AS_OF
    )
    assert got["last_tx_iso"] is None and got["status"] == "NO DATA"


def test_timezone_offsets_are_normalized_to_utc():
    got = fs.FleetSyncRegistry._serve_row(
        2909999,
        _row({"last_tx_field": "JULD_LAST_MESSAGE", "last_tx_iso": "2026-09-11T05:30:00+05:30"}),
        {},
        {},
        AS_OF,
    )
    assert got["last_tx_iso"] == "2026-09-11T00:00:00+00:00"
    assert got["expected_next_iso"] == AS_OF.isoformat()
...
def test_location_time_is_not_profile_measurement_time(tmp_path):
    ...
    assert got["days_since_last_tx"] == 5  # Neither location nor profile time substituted.
...
def test_parser_revision_forces_revalidation_of_same_fingerprints(tmp_path, monkeypatch):
    ...
    fake.downloads.clear()
    reg.sync_now()
    assert fake.downloads == ["2909999_Rtraj.nc", "R2909999_025.nc"]
...
def test_no_comms_is_valid_cached_data_not_endless_failed_download(tmp_path, monkeypatch):
    ...
    assert row["status"] == "NO DATA" and row["profile_count"] == 24
